...
def build(tmp_path):
    ...
    sdlc.create_epic("HIA-TEST", "Feature de démo")
    sdlc.create_ticket("HIA-TEST", "HIA-TEST-2", "socle A", deps=[])
    sdlc.create_ticket("HIA-TEST", "HIA-TEST-3", "socle B", deps=[])
    sdlc.create_ticket("HIA-TEST", "HIA-TEST-1", "API", deps=["HIA-TEST-2", "HIA-TEST-3"],
                       repos=["back-tenant"])
    sdlc.create_ticket("HIA-TEST", "HIA-TEST-4", "UI", deps=["HIA-TEST-1"])
    return sdlc


def test_scaffolding_writes_md(tmp_path):
    build(tmp_path)
    story = tmp_path / "HIA-TEST" / "stories" / "HIA-TEST-1"
    assert (tmp_path / "HIA-TEST" / "prd.md").exists()
...
def test_get_ticket_rehydrates_in_one_call(tmp_path):
    sdlc = build(tmp_path)
    b = sdlc.get_ticket("HIA-TEST-1")
    assert b["id"] == "HIA-TEST-1"
    assert b["repos"] == ["back-tenant"]
    assert b["deps"] == ["HIA-TEST-2", "HIA-TEST-3"]
    assert b["status"] == "draft" and b["next"] == "spec_func"


def test_next_follows_the_dag(tmp_path):
    sdlc = build(tmp_path)
    assert sdlc.next("HIA-TEST") == ["HIA-TEST-2", "HIA-TEST-3"]
    for s in ("spec_tech", "implemented", "reviewed", "deployed", "recette_ok", "accepted", "done"):
        sdlc.set_status("HIA-TEST-2", s)
    for s in ("spec_tech", "implemented", "reviewed", "deployed", "recette_ok", "accepted", "done"):
        sdlc.set_status("HIA-TEST-3", s)
    # socles done -> l'API se débloque
    assert sdlc.next("HIA-TEST") == ["HIA-TEST-1"]
...
def test_full_pipeline_and_board_mirror(tmp_path):
    ...
    for s in steps:
        sdlc.set_status("HIA-TEST-1", s)
    assert sdlc.get_ticket("HIA-TEST-1")["status"] == "done"
    # le board a bien reçu chaque avancement (miroir une-voie)
    assert board.status["HIA-TEST-1"] == "done"
    status_calls = [c for c in board.calls if c[0] == "status" and c[1] == "HIA-TEST-1"]
...
def test_fix_loop_transition(tmp_path):
    ...
    sdlc.set_status("HIA-TEST-1", "implemented")
    assert sdlc.get_ticket("HIA-TEST-1")["status"] == "implemented"


def test_link_artifact(tmp_path):
    sdlc = build(tmp_path)
    sdlc.link_artifact("HIA-TEST-1", "spec_tech", "HIA-TEST/stories/HIA-TEST-1/spec-tech.md")
    assert sdlc.get_ticket("HIA-TEST-1")["artifacts"]["spec_tech"].endswith("spec-tech.md")
