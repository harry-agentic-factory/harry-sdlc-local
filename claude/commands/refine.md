...
Prends le PRD `hia-sdlc/<EPIC>/prd.md`.
...
3. **Repos touchés** par story (cross-repo : back-tenant, plugin, back-hia, hia-ops, front-tenant…).
4. **Écris** `hia-sdlc/<EPIC>/refine.md` (liste stories + `deps:` + ordre suggéré) et **crée** chaque
   ticket :
   `python3 -m sdlc.cli --project HIA create-ticket <EPIC> <STORY> "<titre>" --deps a,b --repos x,y`
5. **Vérifie** le DAG : `python3 -m sdlc.cli --project HIA next <EPIC>` doit renvoyer les stories
