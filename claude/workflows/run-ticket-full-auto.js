...
const reviewPrompt = () => `Story SDLC **${TICKET}** (${WORKREPO}). Review le diff de ${BRANCH} vs ${TARGET} contre les INVARIANTS de ${STORY}/spec-tech.md (+ critères ${STORY}/spec-func.md). Diff: \`git -C ${WORKREPO} diff ${TARGET}...${BRANCH}\`. Vérifie chaque invariant (preuve), cherche bugs/régressions/fuites. Écris ${STORY}/review.md. Ne modifie PAS le code. Dernier message = JSON {conform, note, violations}.`
...
const fixPrompt = (repro) => `Story SDLC **${TICKET}**. Recette KO. Monte l’env local du projet, rejoue le repro (${repro}), corrige sans casser les invariants (${STORY}/spec-tech.md), re-run local jusqu'au vert, commit + push sur ${BRANCH} (depuis le worktree isolé ${WORKREPO}). Dernier message = JSON {fixed, root_cause, commit}.`
...
const cleanPrompt = () => `Story **${TICKET}** mergée sur ${TARGET}. Exécute en Bash \`sdlc --project ${PREFIX} worktree-clean ${TICKET} --ref origin/${TARGET}\` : retire le worktree + la branche **seulement si mergée**, et supprime la bulle régénérable. Renvoie {cleaned, note}.`
